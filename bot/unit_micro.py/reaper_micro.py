...
async def reaper_micro(self):
    for r in self.units(UnitTypeId.REAPER):
        ...
        if r.health_percentage < 2 / 5 and enemyThreatsClose.exists:
            retreatPoints = self.neighbors8(r.position, distance=2) | self.neighbors8(
        ...
        enemyGroundUnits = self.known_enemy_units.not_flying.closer_than(
            5, r
        ...
        reaperGrenadeRange = self._game_data.abilities[
            AbilityId.KD8CHARGE_KD8CHARGE.value
        ]._proto.cast_range
        enemyGroundUnitsInGrenadeRange = (
            self.known_enemy_units.not_structure.not_flying.exclude_type(
